...
Player* CardFourteen::pPlayerOwner = NULL;
...
void CardFourteen::ReadCardParameters(Grid* pGrid) {
	...
	if (price == 0 && fees == 0) {
		out->PrintMessage("New CardFourteen: Enter its price...");
...
void CardFourteen::EditCard(Grid* pGrid) {
	Input* inp = pGrid->GetInput();
	Output* out = pGrid->GetOutput();
...
void CardFourteen::Save(ofstream& OutFile, int Type, string fname)
{
	if (Type != 2)
		return;
...
void CardFourteen::Load(ifstream& InFile, int Type, string fname)
{
	if (Type != 2)
		return;
	int cellNum;
