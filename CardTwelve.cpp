...
Player* CardTwelve::pPlayerOwner = NULL;
...
void CardTwelve::ReadCardParameters(Grid* pGrid) {
	...
	if (price == 0 && fees == 0) {
		out->PrintMessage("New CardTwelve: Enter its price...");
...
void CardTwelve::Save(ofstream& OutFile, int Type, string fname)
{
	if (Type != 2)
		return;
	OutFile << cardNumber << " " << position.GetCellNum() << " " << price << " " << fees << endl;
}
void CardTwelve::Load(ifstream& InFile, int Type, string fname)
{
	if (Type != 2)
		return;
	int cellNum;
