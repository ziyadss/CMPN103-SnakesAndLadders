...
void CardOne::Save(ofstream& OutFile, int Type, string fname)
{
	if (Type != 2)
		return;
	OutFile << cardNumber << " " << position.GetCellNum() << " " << walletAmount << endl;
}
void CardOne::Load(ifstream& InFile, int Type, string fname)
{
	if (Type != 2)
		return;
	int cellNum;
...
void CardOne::EditCard(Grid* pGrid) {
	...
	while (walletAmount < 0) {
		...
		out->PrintMessage("Please enter Positive value : ");
