...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\inputdice.jpg";
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	int cellEndX = GetCellStartX(cellPos) + UI.CellWidth;
	int cellEndY = GetCellStartY(cellPos) + UI.CellHeight;
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int ycoord;
	int num = (fromCell.VCell() - toCell.VCell());
	for (int i = 0; i < num; i++) {
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		ycoord = fromStartY - i * UI.CellHeight;
		pWind->DrawLine(x12, ycoord, x34, ycoord);
	}
}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int xcord[4] = { cellStartX + UI.SnakelineWidth / 4, cellStartX + xChange + UI.SnakelineWidth / 4,cellStartX + 2 * xChange + UI.SnakelineWidth / 4,cellStartX + xChange + UI.SnakelineWidth / 4 };
	int ycord[4] = { fromStartY + 2 * yChange,fromStartY + yChange,fromStartY + 2 * yChange,fromStartY + 3 * yChange };


	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way
	pWind->DrawPolygon(xcord, ycord, 4);
